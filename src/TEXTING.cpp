...
TEXTING::TEXTING(int x, int y, char *string, char *font, int charSize, int width, sf::Color color1, sf::Color color2, sf::Color colorSelection, int type)
{
    ...
    this->tSelection1->loadFromFile("images/seleciona1.png");
    this->tSelection2->loadFromFile("images/seleciona2.png");
...
void TEXTING::actionText(MOUSE *mouses, int *mouse){
    ...
    if(this->type == 0){
        ...
        if( (this->textActivate) && (sf::Mouse::isButtonPressed( sf::Mouse::Left ) ) && (*mouse == 0 ) ){
            *mouse = 1;
            this->choose = true;
            this->clicou = true;
